    ...
    <DashboardLayout>
      <div className="space-y-6 bg-white">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-[#65B32E]">Courses</h1>
        ...
        {defaultCourses.length > 0 && (
          <Card className="border-[#65B32E]/20 bg-white">
            <CardContent className="p-6">
              <div className="mb-4">
                <h2 className="text-xl font-semibold text-[#65B32E]">Default Courses</h2>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {isLoadingDefaultCourses ? (
                  ...
                  defaultCourses.map((course) => (
                    <Card key={course.id} className="border-[#65B32E]/20 bg-white">
                      <CardContent className="p-4">
                        <div className="space-y-3">
                          <div>
                            <h3 className="font-semibold text-[#65B32E]">{course.title}</h3>
                          ...
                          <div className="flex items-center justify-between text-sm text-muted-foreground">
                            ...
                            <Badge variant="outline" className="border-[#65B32E]/30">{course.level}</Badge>
                          </div>
                          <div className="flex gap-2">
                            <Button
        ...
        <div>
          <h2 className="text-xl font-semibold text-[#65B32E] mb-4">Your Courses</h2>
          <Card className="border-[#65B32E]/20 bg-white">
          <CardContent className="p-0">
            <Table>
              <TableHeader>
                <TableRow className="border-[#65B32E]/20 hover:bg-transparent">
                  <TableHead className="text-[#65B32E]">Title</TableHead>
                  <TableHead className="text-[#65B32E]">Students</TableHead>
                  <TableHead className="text-[#65B32E]">Status</TableHead>
                  <TableHead className="text-[#65B32E]">Created</TableHead>
              ...
              <TableBody>
                ...
                {!isLoading && !error && courses.map((course) => (
                  <TableRow 
                    key={course.id} 
                    className="border-[#65B32E]/20 cursor-pointer hover:bg-[#65B32E]/5 transition-colors"
                    onClick={() => router.push(`/org/course/${course.id}`)}
                  >
                    <TableCell className="font-medium text-[#65B32E]">{course.title}</TableCell>
                    <TableCell>{course.enrollments?.length || 0}</TableCell>
                    <TableCell>
                      <Badge variant={course.status === "published" ? "default" : "secondary"} className={course.status === "published" ? "bg-[#65B32E] text-white" : ""}>
                        {course.status}
                      </Badge>
                    </TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-white border-[#65B32E]/20">
                          <DropdownMenuItem asChild className="hover:bg-[#65B32E]/10">
                            <Link href={`/org/course/${course.id}`} className="text-[#65B32E]">
                              <Eye size={16} className="mr-2" />
                              View
                            </Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleOpenEnroll(course)} className="hover:bg-[#65B32E]/10 text-[#65B32E]">
                            <UserPlus size={16} className="mr-2" />
                            Enroll Student
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild className="hover:bg-[#65B32E]/10">
                            <Link href={`/org/course/${course.id}/edit`} className="text-[#65B32E]">
        ...
        <Dialog open={openPreview} onOpenChange={setOpenPreview}>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-white border-[#65B32E]/20">
            <DialogHeader>
              <DialogTitle className="text-[#65B32E]">Course Preview</DialogTitle>
              <DialogDescription>
            ...
            {previewCourse && (
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <h2 className="text-2xl font-bold text-[#65B32E]">{previewCourse.title}</h2>
                  <p className="text-muted-foreground">{previewCourse.description}</p>
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="border-[#65B32E]/30">{previewCourse.level}</Badge>
                ...
                {/* Lessons Section */}
                <div className="space-y-3">
                  <h3 className="text-lg font-semibold text-[#65B32E] flex items-center gap-2">
                    <FileText size={18} />
                  ...
                  {previewCourse.lessons && (previewCourse.lessons as any[]).length > 0 ? (
                    <div className="space-y-2">
                      {(previewCourse.lessons as any[]).map((lesson: any, index: number) => (
                        <Card key={lesson.id || index} className="border-[#65B32E]/20 bg-white">
                          <CardContent className="p-4">
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <h4 className="font-semibold text-[#65B32E]">{lesson.title}</h4>
                                {lesson.content && (
                  ...
                  {previewCourse.quizzes && (previewCourse.quizzes as any[]).length > 0 ? (
                    <div className="space-y-2">
                      {(previewCourse.quizzes as any[]).map((quiz: any, index: number) => (
                        <Card key={quiz.id || index} className="border-[#65B32E]/20 bg-white">
                          <CardContent className="p-4">
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                <h4 className="font-semibold text-[#65B32E]">{quiz.title}</h4>
                                {quiz.description && (
                ...
                {/* Action Buttons */}
                <div className="flex gap-3 pt-4 border-t border-[#65B32E]/20">
                  <Button
                    onClick={() => {
                      setOpenPreview(false)
                      setPreviewCourse(null)
        ...
        <Dialog open={openEnroll} onOpenChange={setOpenEnroll}>
          <DialogContent className="bg-white border-[#65B32E]/20">
            <DialogHeader>
              <DialogTitle className="text-[#65B32E]">Enroll Students in Course</DialogTitle>
              <DialogDescription>
