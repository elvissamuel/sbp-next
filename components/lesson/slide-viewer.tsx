...
export function SlideViewer({ slides, onComplete }: SlideViewerProps) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-black">
              Slide <span className="text-black font-bold">{currentSlideIndex + 1}</span> of <span className="text-gray-900">{sortedSlides.length}</span>
            </span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button
            ...
            disabled={currentSlideIndex === 0}            className="h-9 px-4 text-black hover:text-black hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent transition-all duration-200 border border-gray-600/50 hover:border-[#65B32E] bg-gray-800/30"
          >
      ...
      {/* Slide Content - Gradient Background Presentation Style */}
      <div className="relative bg-gradient-to-br from-gray-900 via-black to-gray-900 rounded-2xl shadow-2xl overflow-hidden min-h-[500px] max-h-[70vh] flex flex-col">
        <div className="flex-1 p-6 md:p-8 flex flex-col">
          ...
          {/* Slide Layout */}
          <div className={`flex-1 ${getLayoutClasses()}`}>
            {/* For split-reverse, show media first, then text */}
            {layout === "split-reverse" ? (
              <>
                {hasMedia && (
                  <div className="flex-1 flex items-center justify-center min-h-[300px]">
                    {media.type === "image" && media.url && (
                      <div className="w-full h-full rounded-lg overflow-hidden bg-gray-800/30 flex items-center justify-center">
                        <img
                          src={media.url}
              ...
              <>
                ...
                {hasContent && (layout !== "media-only") && (
                  <div className={`flex-1 flex flex-col justify-center ${layout === "text-only" ? "w-full min-h-[350px]" : "min-h-[300px]"}`}>
                    <div className="text-gray-200 [&_h1]:text-[#65B32E] [&_h1]:font-bold [&_h2]:text-[#65B32E] [&_h2]:font-bold [&_h3]:text-[#65B32E] [&_h3]:font-bold [&_h4]:text-[#65B32E] [&_h4]:font-bold [&_h5]:text-[#65B32E] [&_h5]:font-bold [&_h6]:text-[#65B32E] [&_h6]:font-bold [&_p]:text-gray-200 [&_p]:text-lg [&_li]:text-gray-200 [&_li]:text-lg [&_strong]:text-[#65B32E] [&_strong]:font-bold [&_em]:text-gray-200 [&_a]:text-[#65B32E] [&_a]:underline [&_a]:hover:text-[#65B32E]/80">
                      <LexicalEditor
                        initialEditorState={content.editorState}
                        editable={false}
                ...
                {hasMedia && (layout !== "text-only") && (
                  <div className={`flex-1 flex items-center justify-center ${layout === "media-only" ? "w-full min-h-[350px]" : "min-h-[300px]"}`}>
                    {media.type === "image" && media.url && (
                      <div className="w-full h-full rounded-lg overflow-hidden bg-gray-800/30 flex items-center justify-center">
                        <img
                          src={media.url}
